...
def play(strategy0, strategy1, score0=0, score1=0, goal=GOAL_SCORE):
    ...
    while score0 < 100 and score1 < 100:
        if who == 0:
            a = select_dice(score0, score1)
            score0 += take_turn(strategy0(score0, score1), score1, a)
        else:
            a = select_dice(score1, score0)
            score1 += take_turn(strategy1(score1, score0), score0, a)
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    max_score = 0
    while i <= 10:
        j = make_averaged(roll_dice)(i, dice)
        if j > max_score:
            k = i
            max_score = j
        i += 1
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=5):
    ...
    NUM_ROLLS otherwise.
    """
    def bacon_pass(score, oppoenet_score):
        a = score + abs(opponent_score // 10 - opponent_score % 10) + 1
        b = 2 * opponent_score
        if a == b or opponent_score == 2 * a:
            if a > opponent_score:
                return False
            return True
        if a - score >= margin:
            return True
    if bacon_pass(score, opponent_score):
        return 0
    return num_rolls
...
max_score = 0
while i <= 10:
    j = make_averaged(roll_dice)(i, six_sided)
    if j > max_score:
        k = i
        max_score = j
    i += 1
...
max_score = 0
while i <= 10:
    j = make_averaged(roll_dice)(i, four_sided)
    if j > max_score:
        k = i
        max_score = j
    i += 1
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    def risk(score, opponent_score):
        return opponent_score - score
    def roll_type(score, opponenet_score):
        if (score + opponent_score) % 7 == 0:
            return False
        return True
    i = swap_strategy(score, opponent_score)
    j = bacon_strategy(score, opponent_score)
    ...
    if i == 0:
        return 0
    if roll_type(score, opponent_score):
        x = bacon_strategy(score, opponent_score, m, b)
        if x == 0:
            return x
        else:
            a = b
    else:
        x = bacon_strategy(score, opponent_score, n, c)
        if x == 0:
            return x
        else:
            a = c
    if score < opponent_score / 2 and score > 0:
        if risk(score, opponent_score) > 70:
            a = 10
        elif risk(score, opponent_score) > 50:
            a = 8
        elif risk(score, opponent_score) > 30:
            a = 7
        elif risk(score, opponent_score) > -50 and risk(score, opponent_score) < -10:
            if d == 0:
                a = 3
            else:
                a = min(3, d)
    return a # Replace this statement
