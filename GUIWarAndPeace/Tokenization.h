...
public:
//------------------------------------------------
    vectorHeap<string> WordHeap[26]; //it will pop words, including duplicates, out in a sorted manner

    vector< node<string> > WordList; //save the popped word, no duplicate
    vector<int> WordCount;           //save the count that pairs up with the word at the same index in the vector WordList
    string* Most10WordList;         //string array that save the 10 most frequent words and their counts

    int netparaNum;             //total paragraph count
    int netlineNum;             //total line count
    int netsentNum;             //total sentence count
    int networdNum;             //total word count
    int networdLength;          //total word length count
    int netnonrepeatwordCount;      //total unique words count
    clock_t t;                      //for calculating the time spent

    string* Abbrev;                 //save the abbreviation database(abbreviated form)
    string* Abbreviations;          //save the abbreviation database(full form)
    string* Contrac;            //save the contraction databse
...
//-------------------functions---------------------
    ...
    string Tokenize(const char* filename);  //call tool functions to analyze the file indicated by user

    float ReadingEase();                        //calculate the reading ease
    string MergeLines(const char* filename);        //merge the original text into a long, modified string, clean up unnecessary symbols
    string Abbreviation(string &line);      //check for abbreviations, if found, convert them to the full form
    string Contraction(string &line);       //identify contraction and words like James' Jack's, do necessary modification
    void Sentences(string &line);           //detect how many sentence are there in the text, clean up punctuations
    void InitialArrays();                   //initial all the arrays in this class


    void InsertInto26Heaps(string &line, int l, int p);     //insert all the word tokens as nodes into 26 heaps, each for words start with one letter
    void InsertToWordList();                            //pop nodes from the 26 heaps in an ordered manner and insert them into one node vector, no duplicate                                                        //also save the count and the locations for each word
    void Find10Words();                                 //find the 10 most frequent words

    void showWordHeap();                                //show the 26 heaps for debugging purpose
    void showWordListAndCount();                        //show the wordlist vector and wordcount vector for debugging purpose
    void WriteToFile(const char *filename);             //write all information into a text file
...
string Tokenization::MergeLines(const char *filename)
{
    ...
    while(getline(file, textline))
    {
        ...
        else
        {
            ...
            while(flag!=-1)
            {
                flag = textline.find_first_of("/0123456789,:;<>()[]{}*+~#$\%^_|");  //replace unnecessary symbols with space
                if(flag != -1)
                    textline.replace(flag, 1, " ");
            }

            flag = 0;
            while(flag!=-1)                                 //formatting some text error
            {
                flag = textline.find("....");
                if(flag != -1)
...
string Tokenization::Abbreviation(string &line)
{
    ...
    {
        if(line == Abbrev[i])                 //check the abbreviation database
            mark = i;
    }
...
string Tokenization::Contraction(string &line)
{
    ...
    {
        if(line == Contrac[i])                  //check the contraction database
            return line;
    }

    int mark = line.length()-1;
    if(line[mark] == 39)                        //James'
        line.erase(mark);


    mark = line.length()-2;
    if((line[mark] == 39) && (line[mark+1] == 's'))     //Jack's
        line.erase(mark, 2);
...
void Tokenization::InsertInto26Heaps(string &line, int l, int p)
{
    string pp;
    ...
    switch(capital)
    {
    case 'A':   WordHeap[0].insert(line, pp);
                break;
    case 'B':   WordHeap[1].insert(line, pp);
                break;
    case 'C':   WordHeap[2].insert(line, pp);
                break;
    case 'D':   WordHeap[3].insert(line, pp);
                break;
    case 'E':   WordHeap[4].insert(line, pp);
                break;
    case 'F':   WordHeap[5].insert(line, pp);
                break;
    case 'G':   WordHeap[6].insert(line, pp);
                break;
    case 'H':   WordHeap[7].insert(line, pp);
                break;
    case 'I':   WordHeap[8].insert(line, pp);
                break;
    case 'J':   WordHeap[9].insert(line, pp);
                break;
    case 'K':   WordHeap[10].insert(line, pp);
                break;
    case 'L':   WordHeap[11].insert(line, pp);
                break;
    case 'M':   WordHeap[12].insert(line, pp);
                break;
    case 'N':   WordHeap[13].insert(line, pp);
                break;
    case 'O':   WordHeap[14].insert(line, pp);
                break;
    case 'P':   WordHeap[15].insert(line, pp);
                break;
    case 'Q':   WordHeap[16].insert(line, pp);
                break;
    case 'R':   WordHeap[17].insert(line, pp);
                break;
    case 'S':   WordHeap[18].insert(line, pp);
                break;
    case 'T':   WordHeap[19].insert(line, pp);
                break;
    case 'U':   WordHeap[20].insert(line, pp);
                break;
    case 'V':   WordHeap[21].insert(line, pp);
                break;
    case 'W':   WordHeap[22].insert(line, pp);
                break;
    case 'X':   WordHeap[23].insert(line, pp);
                break;
    case 'Y':   WordHeap[24].insert(line, pp);
                break;
    case 'Z':   WordHeap[25].insert(line, pp);
                break;
...
void Tokenization::showWordHeap()
{
    node<string> temp;
    int count = 0, i=0;
...
//                if(!(count%5))
//                    cout<<endl;
//                cout<<setw(16)<<temp.data << " [" << temp.LinePara << "]";
//                count++;
            }
            catch(ERRORS e)
            {
...
{
    node<string> temp;
    int i=0, j=0, mark=0, searchMark = 0, lastMark = 0, count = 0;
    bool found;


    for(i=0;i<26;i++)
    {
//        cout <<"      =====processing WordHeap["
//             << i << "]=====" <<endl;
        count = 0;
        ...
        while(!WordHeap[i].empty())
        {
            try
            {
                WordHeap[i]>>temp;  //temp is a node, not a node pointer

                if(WordList.empty() && WordCount.empty())
                {
                ...
                {
                    //since wordlist is sorted, if the first letter are the same with the new item,
                    //update searchMark in order to cut off searching time

                    if(WordList[lastMark].data[0] != temp.data[0])
                        searchMark = lastMark;

                    found = false;
                    for(j=searchMark; j<=lastMark;j++)
                    {
                        if(WordList[j].data == temp.data)
                        {
                            found = true;
                            mark = j;
                        }
                    ...
                    if(found)
                    {
                        WordList[mark].LinePara += "  ";
                        WordList[mark].LinePara += temp.LinePara;
                        WordCount[mark]++;
...
void Tokenization::InitialArrays()
{
    ...
    while(getline(AbbrevFile, buffer))      //reading the database text file
    {
        if(buffer.empty())
        {
            //do nothing
    ...
    while(getline(AbbreviationFile, buffer))        //reading the database text file
    {
        if(buffer.empty())
        {
            //do nothing
...
//------------------------determine how many sentence there are, remove . ... ? ! "----------------------------------------
...
//-----------------------------------------find contraction and convert----------------------------------------
            ...
            if(tempstr.find("'") != -1)  //find '
                tempstr = Contraction(tempstr);

//-------------------------------------------------put tokens into 26 heaps-----------------------------------------------------
            InsertInto26Heaps(tempstr, linePos, paraPos);
        }
