	...
	def remove_insect(self, insect):
		...
		if insect.is_ant:
			...
			if insect.container and insect.ant:
				self.ant = insect.ant
				self.ant.place = self
			elif type(insect) == QueenAnt and insect.total_queens == 1:
				return
			else:
				self.ant = None
		else:
...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		while current_place !=  hive:
			if random_or_none(current_place.bees) and self.min_range <= i <= self.max_range:
				return random_or_none(current_place.bees)
			current_place, i = current_place.entrance, i + 1
		return None
...
class WallAnt(Ant):
	"""A WallAnt that cannot move but have 4 armor."""
...
class ScubaThrower(ThrowerAnt):
	"""A ThrowerAnt that is watersafe and cost more food."""
	name = 'Scuba'
	food_cost = 5
	watersafe = True
	implemented = True
...
class HungryAnt(Ant):
	...
	def action(self, colony):
		if random_or_none(self.place.bees) is not None and self.digesting == 0:
...
class QueenAnt(ScubaThrower):  # You should change this line
	...
	def __init__(self, armor=1):
		...
		self.total_queens = QueenAnt.total_queens
		self.armor = armor
	...
	def action(self, colony):
		...
		if self.total_queens == 1:
			ScubaThrower.action(self,colony)
		def double(place, exit, entrance):
			if place.ant and place.ant not in self.doubled and type(place.ant) != QueenAnt:
				place.ant.damage *= 2
				self.doubled.append(place.ant)
				if place.ant.container:
					if place.ant.ant and type(place.ant.ant) != QueenAnt:
						place.ant.ant.damage *= 2
						self.doubled.append(place.ant.ant)
			if place.exit and exit and not entrance:
				double(place.exit, exit, entrance)
			elif place.entrance and entrance and not exit:
				double(place.entrance, exit, entrance)
			elif exit and entrance:
				if place.exit:
					double(place.exit, exit, False)
				if place.entrance:
					double(place.entrance, False, entrance)
		if self.total_queens > 1:
			self.reduce_armor(self.armor)
		else:
			double(self.place, True, True)
