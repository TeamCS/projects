...
class Place:
	...
	def remove_insect(self, insect):
		...
		if insect.is_ant:
			...
			elif type(insect) == QueenAnt and not insect.imposter:
				return
			else:
				self.ant = None
		else:
...
class ScubaThrower(ThrowerAnt):
	"""A ThrowerAnt taht is watersafe and cost more food."""
	name = 'Scuba'
	food_cost = 5
	watersafe = True
	implemented = True
...
class QueenAnt(ScubaThrower):
	...
	def action(self, colony):
		...
		for place in self.tunnel:
			if place.ant is not None and place.ant not in self.double_ant and type(place.ant) != QueenAnt:
				place.ant.damage = 2 * place.ant.damage
				self.double_ant.append(place.ant)
				if place.ant.container:
					if place.ant.ant and type(place.ant.ant) != QueenAnt:
						place.ant.ant.damage = 2 * place.ant.ant.damage
						self.double_ant.append(place.ant.ant)
					
	@property	
	def tunnel(self):
		left, right = self.place, self.place
		l, r = [], []
		while left.exit != None:
			l.append(left.exit)
			left = left.exit
		while right.entrance != None:
			r.append(right.entrance)
			right = right.entrance
		return l + r + [self.place]
