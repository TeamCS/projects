...
def do_define_form(vals, env):
	...
	if scheme_symbolp(target):
		...
		func_name, formals, body = target.first, target.second, vals.second
		if not scheme_symbolp(func_name):
			raise SchemeError("bad argument to define")
		env.define(func_name, do_lambda_form(Pair(formals, body), env))
		return func_name
	else:
		raise SchemeError("bad argument to define")
...
def do_let_form(vals, env):
	...
	for i in range(len(bindings)):
		...
		name, value = bindings[i][0], scheme_eval(bindings[i][1], env)
		if not scheme_symbolp(name):
			raise SchemeError('bad binding: ({0} {1})'.format(name, value))
		elif name in check_name:
			raise SchemeError('duplicate binding: {0}'.format(name))
...
def do_if_form(vals, env):
	...
	check_form(vals, 2, 3)
	pred, conseq = scheme_eval(vals[0], env), vals[1]
	if len(vals) == 2:
		alt = okay
	else:
		alt = vals[2]
...
def do_and_form(vals, env):
	...
	while vals is not nil:
		evaled = scheme_eval(vals.first, env)
		if scheme_false(evaled):
			return False
...
def check_formals(formals):
	...
	for formal in formals:
		if not scheme_symbolp(formal):
			raise SchemeError('not a symbol: {0}'.format(formal))
		if formal in formal_lst:
			raise SchemeError('repeated symbol in formal parameters: {0}'.format(formal))
		formal_lst.append(formal)
