...
(define (list-partitions total max-pieces max-value)
  (define (helper total max-pieces max-value results)
	(cond
		((and (= total 0) (>= max-pieces 0)) (list results))
		((or (= max-value 0) (= max-pieces 0)  (< total 0)) '())
		(else (append (helper (- total max-value) (- max-pieces 1) max-value (append results (list max-value)))
					  (helper total max-pieces (- max-value 1) results)))))
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               ...
               (body   (cddr expr)))
          (cons form 
				(cons (apply-to-all analyze params)
